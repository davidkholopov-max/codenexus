...
import { Eye, EyeOff, Code2, Github, Loader2, Zap, CheckCircle2, Mail } from 'lucide-react'
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ name: data.name, email: data.email, password: data.password, locale }),
      })
      if (res.ok) {
        // Sign in immediately so the session is active before onboarding
        await signIn('credentials', {
          email: data.email,
          password: data.password,
          redirect: false,
        })
        setRegistered({ name: data.name, email: data.email })
      } else {
  ...
  // Success screen shown after registration + auto sign-in
  if (registered) {
    return (
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-card border border-border rounded-2xl p-8 shadow-xl shadow-black/10 text-center">
            ...
            <p className="text-lg font-medium text-primary mb-3">
              {t('successGreeting', { name: registered.name })}
            </p>
            ...
            {/* Email notice */}
            <div className="flex items-center gap-3 bg-muted/50 border border-border rounded-xl px-4 py-3 mb-8 text-left">
              <Mail className="w-4 h-4 text-muted-foreground shrink-0" />
              <p className="text-xs text-muted-foreground leading-relaxed">
                {t('successEmail', { email: registered.email })}
              </p>
            </div>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card border border-border rounded-2xl p-8 shadow-xl shadow-black/10">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button
              type="button"
              onClick={() => signIn('github', { callbackUrl: `/${locale}/onboarding` })}
              className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-border hover:bg-accent/50 transition-colors text-sm font-medium"
            >
